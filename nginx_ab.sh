...
single_concurrency=200
step_concurrency=100
server_ip=192.168.10.3
array="1K 10K 100K 1M"
...
while (( $single_concurrency<$target_concurrency ))
	do
	echo "ab -n $totalcycle -c $single_concurrency http://$server_ip:8080/$single_array"
	ab -n $totalcycle -c $single_concurrency http://$server_ip:8080/$single_array >> ./$date_dir/$single_array.log	
